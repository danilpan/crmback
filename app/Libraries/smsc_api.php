...
{
	if (isset($params)) {
		if(isset($params['login']))$this->smsc_login = $params['login'];			// логин клиента
		if(isset($params['password']))$this->smsc_password = $params['password'];	// пароль
		if(isset($params['from']))$this->smsc_from = $params['from'];     // e-mail адрес отправителя
		if(!defined('SMSC_POST'))define("SMSC_POST", 0);					// использовать метод POST
		if(!defined('SMSC_HTTPS'))define("SMSC_HTTPS", 0);				// использовать HTTPS протокол
		if(!defined('SMSC_CHARSET'))define("SMSC_CHARSET", "utf-8");	// кодировка сообщения: utf-8, koi8-r или windows-1251 (по умолчанию)
		if(!defined('SMSC_DEBUG'))define("SMSC_DEBUG", 0);				// флаг отладки
	}else{
		exit('Не переданы данные для входа.');
	};
}
...
{
	$url = $_url = (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode($this->smsc_login)."&psw=".urlencode($this->smsc_password)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
