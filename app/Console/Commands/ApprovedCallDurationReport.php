...
class ApprovedCallDurationReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ApprovedCallDurationReport:send';
     ...
     * @var string
     */
    protected $description = 'Отчёт продолжительности апрувленного разговора';
    ...
    {
        $message = "Отчёт продолжительности аппрувленного разговора:\n";
        $main_kc_id = env('MAIN_KC_ID', 3); //ID главного коллцентра
        $kc_child = Organization::where('parent_id', $main_kc_id)->get();
        ...
        $data = DB::table('orders as o')
            ...
            ->join('order_status as os', function ($jin) {
                $jin->on('os.order_id', '=', 'o.id')
                    ->where([['os.status_type', 1], ['os.status_id', 17]]);
            })
